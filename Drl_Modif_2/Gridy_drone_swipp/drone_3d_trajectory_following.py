...
def quad_sim(x_c, y_c, z_c, taille, X , Y, Z, ox, oy, oz):
    """
    ...
    q = Quadrotor( mx, my, mz, ox, oy, oz, x=x_pos, y=y_pos, z=z_pos, roll=roll,
    ...
    while True:
        while t <= T:
            ...
            q.update_pose(x_pos, y_pos, z_pos, roll, pitch, yaw, mx, my, mz, ox, oy, oz)
...
def Drone(px, py, pz, ox, oy, oz):
    ...
    for i in range(0, taille):
        ...
        z_coeffs.append([])
        waypoints.append([])
        waypoints[i].append(px[i])
        waypoints[i].append(py[i])
        waypoints[i].append(pz[i])


    
    
    for i in range(0, taille):
        traj = TrajectoryGenerator(waypoints[i], waypoints[(i + 1) %taille], T)
        traj.solve()
        x_coeffs[i] = traj.x_c
        y_coeffs[i] = traj.y_c
        z_coeffs[i] = traj.z_c

    quad_sim(x_coeffs, y_coeffs, z_coeffs,taille, px, py, pz, ox, oy, oz)


if __name__ == "__Drone__":
    Drone()
